...
/**
  ...
  */
abstract class ModelCoordinator(val initialTime: Duration, randomActor: ActorRef, val simLogger: ActorRef) extends LoggingActor with UniqueNames {
   ...
   */
  protected var externalEvents: List[ExternalEvent[Serializable]] = List()
  ...
  /**
   * Call this method from the [[handleExternalEvent()]] method in order to get a unique index to assign to an [[EventMessage]]
  ...
  /**
    ...
    */
  def sendEventMessage[E <: Serializable](externalEvent: ExternalEvent[E], receiveActor: ActorRef): Unit = {
    val eventMessage = new EventMessage(externalEvent, externalEvent.executionTime, nextEventIndex)
    awaitingBagEvents = AwaitingEventBagging(receiveActor, eventMessage.eventIndex) :: awaitingBagEvents
    influences = receiveActor :: influences
    val t = eventMessage.t
  ...
  /**
    * Event messages cannot be send directoy to subordinate [[ModelSimulator]]s.  This utility method must be
    * used so that the [[awaitingBagEvents]] and [[influences]] lists are updated properly.
   * @param eventData  The passed in data to be wrapped in the [[EventMessage]]
   * @param t The time the [[EventMessage]] is sent
   * @param receiveActor The receiver of the EventMessage
   */
  def sendEventMessage[T <: Serializable](eventData: T, t: Duration, receiveActor: ActorRef): Unit = {
    sendEventMessage(new ExternalEvent(t, eventData), receiveActor)
  }
  ...
  private def checkDonePassingMessages(t: Duration): Unit = {
    ...
    if (awaitingBagEvents.isEmpty) {
      ...
      if (synchronizeSet.isEmpty) {
        ...
        synchronizeSet.foreach(s => s ! ExecuteTransition(currentTime))
        imminents = List()
        influences = List()
   ...
   */
  def handleOutputEvent(eventSender: ActorRef, output: OutputMessage[_])
   ...
   */
  def handleExternalEvent[E <: Serializable](externalEvent: ExternalEvent[E])
  ...
  def receive = {
    case GetNextTime() =>
      ...
      nextMap.keySet.foreach(_ ! GetNextTime())
  ...
  def processingOutput: Receive = {
    ...
    case outputMessage: OutputMessage[_] =>
      if (outputMessage.t.compareTo(currentTime) == 0) {
        logDebug(outputMessage.t + " Handling output event " + outputMessage.output + " from " + sender().path.name)
        handleOutputEvent(sender(), outputMessage)
      }
      else {
        throw new SynchronizationException(outputMessage.t + " in processing output, time in OutputMessage " + outputMessage.t + " does not match current time: " + currentTime)
      }

    case BagEventDone(t, eventIndex) =>
    ...
    case p: ProcessEventMessages =>
      ...
      logDebug(p.t + " Received ProcessEventMessages from " + sender + " Sending to subordinates.")
    ...
    case Terminate() =>
      ...
      subordinates().foreach(_ ! Terminate())
   ...
   * completed state transition.  Set the current time to [[TransitionDone.t]] and send a [[TransitionDone]] message
  ...
  def processingTransitions: Receive = {
    case TransitionDone(t, nextTime) =>
      if (t.compareTo(currentTime) >= 0 && t.compareTo(nextTime) <= 0) {
        ...
        if (synchronizeSet.isEmpty) {
          currentTime = t
          context.parent ! TransitionDone(currentTime, getNextTime)
