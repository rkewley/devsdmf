 ...
 *
   ...
   */
  protected var externalEvents: List[ExternalEvent[GeneratedMessage]] = List()
 ...
 *
    ...
    */
  def sendEventMessage[T <: GeneratedMessage](externalEvent: ExternalEvent[T], receiveActor: ActorRef): Unit = {
    val devsEventData = ModelSimulator.buildDEVSEventData(DEVSEventData.EventType.EXTERNAL, externalEvent.executionTime, externalEvent.eventData)
    val eventMessage = ModelSimulator.buildEventMessage(devsEventData, externalEvent.executionTime, nextEventIndex)
    awaitingBagEvents = AwaitingEventBagging(receiveActor, eventMessage.getEventIndex) :: awaitingBagEvents
 ...
 *
   ...
   */
  def sendEventMessage[T <: GeneratedMessage](eventData: T, t: Duration, receiveActor: ActorRef): Unit = {
 ...
 *
   ...
   */
  def handleOutputEvent(eventSender: ActorRef, output: OutputMessage)
   ...
   */
  def handleExternalEvent[E <: GeneratedMessage](externalEvent: ExternalEvent[E])
 ...
 *
  ...
  def passingMessages: Receive = {
    ...
    case em: EventMessage => //(externalEvent, t, eventIndex)
      val t: Duration = Duration.parse(em.getTimeString)
      val executionTime: Duration = Duration.parse(em.getEvent.getExecutionTimeString)
      val externalEvent = new ExternalEvent(executionTime, convertEvent(em.getEvent.getEventData))
      externalEvents = externalEvent :: externalEvents
